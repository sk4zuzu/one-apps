...
module WireGuard
    ...
    ONEAPP_VNF_WG_INTERFACES_OUT = env :ONEAPP_VNF_WG_INTERFACES_OUT, '' # nil -> none, empty -> all
    ...
    def parse_env
        ...
        text = bash "isoinfo -i #{iso_path} -R -x #{cfg_path}", chomp: true

        if text.empty?
            msg :error, 'WireGuard config looks empty!'
            { cfg: {} }
        else
            { cfg: YAML.load(text) }
        end
    ...
    def install(initdir: '/etc/init.d')
        ...
        puts bash 'apk --no-cache add cdrkit wireguard-tools-wg-quick'
    ...
    def configure(basedir: '/etc/wireguard')
        ...
        parse_env[:cfg].each do |dev, opts|
            unless @interfaces.include?(opts['interface_out'])
                msg :error, "Forbidden out interface: #{opts['interface_out']}"
                next
            end
